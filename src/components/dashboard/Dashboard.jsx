...
const getAccessibleUnits = (plantId) => {
  ...
  const [totalWashDelivery, setTotalWashDelivery] = useState(0);
  const [totalWashReceive, setTotalWashReceive] = useState(0);
  ...
  const fetchTotalWashDelivery = async ({ showLoading = true } = {}) => {
    if (!filters.fromDate) return;
    try {
      const params = {
        fromDate: filters.fromDate,
      ...
      if (filters.unit) {
        params.washUnit = [filters.unit];
      }
      const response = await dashboardApi.getWashDelivery(params);
      if (response?.success && Array.isArray(response.data) && response.data.length > 0) {
        setTotalWashDelivery(response.data[0].delivery || 0);
        setTotalWashReceive(response.data[0].receive || 0);
      } else {
        setTotalWashDelivery(0);
        setTotalWashReceive(0);
      }
    } catch (err) {
      console.error('Total wash delivery fetch error:', err);
    }
  };
  ...
  useEffect(() => {
    ...
    const refreshDashboard = (options) => {
      ...
      fetchTotalWashDelivery(options);
    };
...
`}</style>
     ...
     <div className="
      ...
      flex flex-col gap-2">
        ...
        {/* Top Summary Cards */}
        {!loading && !error && (
          <TopSummaryCards isDarkMode={isDarkMode} dashboardData={dashboardData} totalWashDelivery={totalWashDelivery} totalWashReceive={totalWashReceive} onCardClick={openModal} />
        )}
...
const TopSummaryCards = ({ isDarkMode, dashboardData, totalWashDelivery = 0, totalWashReceive = 0, onCardClick }) => {
  const cards = [
    {
      title: '1st Dry  ',
      type: 'blue',
    ...
    },
    {
      ...
      type: 'blue',
      icon: 'water',
      received: totalWashReceive,
      delivery: totalWashDelivery,
      showBoth: true,
    },
  ];
      ...
      {cards.map((card, index) => (
        <TopCard
          ...
          onCardClick={() => onCardClick({
            processStageIds: card.title.trim() === 'Total Delivery' ? [] : (PROCESS_STAGE_IDS[card.title.trim()] || [1]),
            modalTitle: card.title.trim(),
            mode: card.title.trim() === 'Total Delivery' ? 'wash-delivery' : 'process',
          })}
        />
      ))}
    </div>
...
const TopCard = ({ card, isDarkMode, onCardClick }) => {
  ...
  return (
    <div
      onClick={card.noClick ? undefined : onCardClick}
      className={`border-2 rounded-2xl p-1.5 md:p-2 transition-all duration-300 ${card.noClick ? '' : 'cursor-pointer hover:-translate-y-1 hover:shadow-xl'} ${
        isDarkMode
          ? isOrange
