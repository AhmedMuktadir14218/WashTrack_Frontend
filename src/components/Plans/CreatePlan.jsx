...
const CreatePlan = () => {
  ...
  useEffect(() => {
    if (showPlannedTab && selectedProcessStage && isDryStage) {
      fetchWashPlans();
    }
  }, [showPlannedTab, selectedProcessStage, planDate, shift]);
  ...
  const fetchWashPlans = async () => {
    try {
      ...
      const params = {};
      if (planDate) params.FromDate = planDate;
      if (planDate) params.ToDate = planDate;
      if (shift) params.Shift = parseInt(shift);
      if (plantId) params.PlantId = parseInt(plantId);
      if (unitId) params.UnitId = parseInt(unitId);
      
      // ✅ Fetch ONLY for the selected Process Stage
      ...
      if (response.data?.success) {
        ...
        // ✅ Strict Filter to prevent cross-stage leakage
        const stageFilteredRecords = records.filter(r => r.processStageId === selectedProcessStage?.processStageId);
        
        setWashPlans(stageFilteredRecords);
      ...
      const planData = planItems.map(item => {
        if (isDryStage) {
          return {
            ...
            percentage: 0,
            finalTargetQty: item.finalTargetQty || item.baseTargetQty || 0,
            adjustedTargetQty: item.finalTargetQty || item.baseTargetQty || 0,
  ...
  const filteredWashPlans = washPlans.filter(wp => {
    if (planDate && wp.planDate !== planDate) return false;
    if (shift && wp.shift !== parseInt(shift)) return false;
    if (selectedProcessStage && wp.processStageId !== selectedProcessStage.processStageId) return false;
    return true;
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
              ...
              {!plantId || !unitId ? (
                ...
                <>
                  <div ref={tableScrollRef} className="flex-1 overflow-auto custom-scrollbar">
                    <table className="w-full text-sm" style={{ minWidth: isWashStage ? '950px' : '750px' }}>
                        ...
                        {workOrders.map((order, idx) => {
                          ...
                          return (
                            <tr key={`${order.workOrderNo}-${idx}`} className={`transition-colors ${added ? 'bg-indigo-50/40' : 'hover:bg-gray-50'}`}>
                              ...
                              <td className="px-3 py-2">
                                <div className="text-xs text-gray-900 font-medium truncate max-w-[100px]">{order.styleName}</div>
                                <div className="text-[10px] text-gray-500 mt-0.5 truncate max-w-[100px]">{order.color}</div>
                              </td>
                              <td className="px-3 py-2">
                                <div className="text-xs text-gray-700 truncate max-w-[80px]">{order.buyer}</div>
                                <div className="text-[10px] text-gray-400 mt-0.5 truncate max-w-[80px]">{order.marks || '-'}</div>
                              </td>
                  ...
                  <div className="px-4 py-2.5 border-t border-gray-200 bg-gray-50 flex items-center justify-between flex-shrink-0">
                    <div className="text-xs text-gray-500">
                      {modalPagination.totalRecords > 0 && <span>Showing {workOrders.length} of {modalPagination.totalRecords}</span>}
                      {hasMoreResults && (
                        <button onClick={handleLoadMore} disabled={searchLoading} className="ml-3 text-indigo-600 hover:text-indigo-700 font-semibold disabled:opacity-50">
            ...
            <div className="flex flex-col flex-1 min-h-0">
              <div className="px-4 py-2.5 border-b border-gray-100 flex-shrink-0">
                <p className="text-xs text-gray-500">
                  Showing wash plans assigned to <strong className="text-gray-800">{selectedProcessStage?.processStageName}</strong> matching date & shift. Complete the fields to submit.
                  {!planDate && <span className="text-amber-600 font-semibold ml-1">Please select a Plan Date above.</span>}
                  {!shift && <span className="text-amber-600 font-semibold ml-1">Please select a Shift above.</span>}
                </p>
              </div>
              {washPlansLoading ? (
                <div className="flex-1 flex items-center justify-center">
                ...
                <>
                  <div className="flex-1 overflow-auto custom-scrollbar">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 sticky top-0 z-10">
                        <tr>
                          <th className="px-3 py-2.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider w-8"></th>
                          <th className="px-3 py-2.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider">WO No</th>
                          <th className="px-3 py-2.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider w-28">Style / Color</th>
                          <th className="px-3 py-2.5 text-center text-[10px] font-bold text-gray-500 uppercase tracking-wider w-20">Base Target</th>
                          {/* ✅ Machine Column Removed completely for Dry Stage */}
                          <th className="px-3 py-2.5 text-center text-[10px] font-bold text-gray-500 uppercase tracking-wider w-20">Final Target</th>
                          <th className="px-3 py-2.5 text-center text-[10px] font-bold text-gray-500 uppercase tracking-wider w-14">Action</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100">
                        {filteredWashPlans.map((wp, idx) => {
                          ...
                          return (
                            <tr key={`${wp.id || wp.workOrderId}-${idx}`} className={`${alreadyAdded ? 'bg-green-50/50' : 'hover:bg-gray-50'} transition-colors`}>
                              ...
                              <td className="px-3 py-2"><span className="font-bold text-indigo-600 text-xs">{wp.workOrderNo}</span></td>
                              <td className="px-3 py-2">
                                <div className="text-xs text-gray-900 font-medium truncate max-w-[100px]" title={wp.styleName}>{wp.styleName}</div>
                                <div className="text-[10px] text-gray-500 truncate max-w-[100px]" title={wp.color}>{wp.color}</div>
                              </td>
                              
                              {/* Display the Base Target set by the previous wash plan */}
                              <td className="px-3 py-2 text-center text-xs font-bold text-blue-700 bg-blue-50/30 rounded border border-blue-100">{wp.baseTargetQty?.toLocaleString() || '-'}</td>
                  ...
                  <div className="px-4 py-2.5 border-t border-gray-200 bg-gray-50 flex items-center justify-between flex-shrink-0">
                    <div className="text-xs text-gray-500">
                      Showing {filteredWashPlans.length} wash plans matching date & shift
                    </div>
                    {selectedCount > 0 && (
                      <button onClick={() => setActiveTab('selected')}
                        className="px-4 py-1.5 bg-indigo-600 text-white text-xs font-semibold rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-1.5">
                        ...
                        {planItems.map(item => {
                          ...
                          return (
                            <tr key={item.workOrderNo} className={`${incomplete ? 'bg-amber-50/50' : 'hover:bg-gray-50'} transition-colors`}>
                              ...
                              <td className="px-3 py-2">
                                <div className="text-xs text-gray-900 font-medium truncate max-w-[100px]">{item.styleName}</div>
                  ...
                  {hasIncompleteItems && (
                    <div className="mt-3 p-2.5 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2 flex-shrink-0">
                      ...
                      <p className="text-xs font-semibold text-amber-800">
                        Incomplete items — go back to Work Orders tab to fill missing fields (Base Target or Machines).
                      </p>
                    </div>
                  )}
      ...
      {openDropdownFor && machines.length > 0 && !isDryStage && (
        <div id="machine-dropdown" className="fixed z-[9999] w-72 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden" style={{ top: dropdownPos.top, left: dropdownPos.left }}>
          ...
          <div className="max-h-48 overflow-y-auto custom-scrollbar">
            {getFilteredMachines(openDropdownFor).length === 0 ? (
              <div className="px-3 py-4 text-xs text-gray-400 text-center">No machines found</div>
            ) : (
...
//   // ★ Machine optional when user has BOTH wash+dry access AND is on dry stage
//   const isMachineOptional = isDryStage && hasWashPermission;

//   // ── Calculation Logic ──
//   const calculateBaseTarget = (shiftVal, machineQty, cycleTime, batchQty) => {
//     if (!shiftVal || machineQty === 0 || !cycleTime || !batchQty) return 0;
//     const shiftHours = parseInt(shiftVal) === 1 ? 11 : 12;
//     return Math.floor(((shiftHours * 60) * machineQty) / cycleTime) * batchQty;
//   };
...
//     }
//   };
// const hideMachineField = isDryStage;
//   const fetchMachines = async () => {
//     try {
//       setLoading(true);
//       const response = await plansApi.getMachines(plantId, unitId);
...
//       // ✅ API-তে সরাসরি ProcessStageId পাঠিয়ে দিচ্ছি
//       if (selectedProcessStage) {
//         params.ProcessStageId = selectedProcessStage.processStageId;
//       }
...
//         // ✅ Frontend-এও strict filter করে দিচ্ছি যেন অন্য Stage না আসে
//         const stageFilteredRecords = records.filter(r => 
...
//       const processStageId = selectedProcessStage?.processStageId;
//       if (processStageId === 4) {
//         cycleTimeValue = parseFloat(order.firstWashBatchTime) || 0;
//         batchQtyValue = parseFloat(order.firstWashBatchQty) || 0;
//       } else if (processStageId === 5) {
//         cycleTimeValue = parseFloat(order.secondWashBatchTime) || 0;
//         batchQtyValue = parseFloat(order.secondWashBatchQty) || 0;
//       }
//       if (cycleTimeValue && batchQtyValue && shift) {
//         const machineQty = 0;
//         baseTargetDefault = calculateBaseTarget(shift, machineQty, cycleTimeValue, batchQtyValue);
...
//   // Add from Planned tab (dry-only users)
//   const addFromPlanned = (wp) => {
//     if (isInPlan(wp.workOrderNo)) return;
...
//         selectedMachines: newSelectedMachines
//       };
//       // Recalculate for wash stage when machines change
//       if (isWashStage) {
//         const machineQty = newMachineIds.length;
//         const base = calculateBaseTarget(shift, machineQty, updated.cycleTime, updated.batchQty);
//         const adjusted = calculateAdjusted(base, updated.percentage);
//         updated.baseTargetQty = base;
//         updated.adjustedTargetQty = adjusted;
//         updated.finalTargetQty = adjusted;
//       }
//       return updated;
//     }));
//   };
...
//   // ── Shift change recalculation for wash stage ──
//   useEffect(() => {
//     if (isWashStage && shift && planItems.length > 0) {
//       setPlanItems(prev => prev.map(item => {
//         const machineQty = item.machineIds?.length || 0;
//         const base = calculateBaseTarget(shift, machineQty, item.cycleTime, item.batchQty);
//         const adjusted = calculateAdjusted(base, item.percentage);
//         return { ...item, baseTargetQty: base, adjustedTargetQty: adjusted, finalTargetQty: adjusted };
//       }));
//     }
//   }, [shift, isWashStage]);
...
//       if (isMachineOptional) {
//         // Both accesses + Dry Stage: Only Base Target is required
//         if (!item.baseTargetQty || item.baseTargetQty <= 0) {
//           return toast.error(`Enter base target for ${item.workOrderNo}`);
//         }
//       } else if (showPlannedTab) {
//         // Dry-only permission: Machines, Base Target AND Final Target required
//         if (item.machineIds.length === 0) {
//           return toast.error(`Select machine(s) for ${item.workOrderNo}`);
//         }
...
//         }
//         if (!item.baseTargetQty || item.baseTargetQty <= 0) {
//           return toast.error(`Base target missing for ${item.workOrderNo}`);
//         }
//       } else {
//         // Wash Stage: Machines and Base Target required
//         if (item.machineIds.length === 0) {
...
//       setSubmitting(true);
//       const planData = planItems.map(item => {
//         // For dry stage (both perms): percentage=0, adjusted=baseTarget, finalTarget=baseTarget (if not explicitly set otherwise)
//         if (isMachineOptional) {
//           return {
//             workOrderId: item.workOrderId,
//             processStageId: selectedProcessStage.processStageId,
...
//             baseTargetQty: item.baseTargetQty,
//             percentage: 0,
//             finalTargetQty: item.baseTargetQty,
//             adjustedTargetQty: item.baseTargetQty,
//             remarks: item.remarks || '',
//             isDeleted: false,
//             createdBy: user?.id || 0,
//             createdAt: new Date().toISOString()
...
//         // For dry-only access (Planned tab used): percentage=0, adjusted=finalTarget, finalTarget=finalTarget
//         if (showPlannedTab) {
//            return {
//             workOrderId: item.workOrderId,
//             processStageId: selectedProcessStage.processStageId,
...
//             percentage: 0,
//             finalTargetQty: item.finalTargetQty,
//             adjustedTargetQty: item.finalTargetQty,
//             remarks: item.remarks || '',
//             isDeleted: false,
//             createdBy: user?.id || 0,
//             createdAt: new Date().toISOString()
...
//         // For wash stage: send all calculated values
//         return {
//           workOrderId: item.workOrderId,
//           processStageId: selectedProcessStage.processStageId,
//           planDate,
...
//   // ★ Validation for incomplete items adapts to user permission
//   const hasIncompleteItems = planItems.some(i => {
//     if (isMachineOptional) {
//       return !i.baseTargetQty || i.baseTargetQty <= 0;
//     } else if (showPlannedTab) {
//       return !i.baseTargetQty || i.baseTargetQty <= 0 || !i.finalTargetQty || i.finalTargetQty <= 0 || i.machineIds.length === 0;
//     } else {
//       return (!i.machineIds || i.machineIds.length === 0) || !i.baseTargetQty || i.baseTargetQty <= 0;
//     }
//   });
...
//   // Filter wash plans for planned tab based on date, shift, process stage
//   const filteredWashPlans = washPlans.filter(wp => {
//     // ✅ Date Filter
//     if (planDate && wp.planDate !== planDate) return false;
...
//     // ✅ Strict Process Stage Filter (1st Dry হলে শুধু 1st Dry)
//     if (selectedProcessStage && wp.processStageId !== selectedProcessStage.processStageId) return false;
...
//             </div>
//             <div className="flex items-center gap-2">
//               {/* ★ Show hint when machine is optional */}
//               {isMachineOptional && (
//                 <span className="hidden sm:inline-flex items-center gap-1 px-2 py-1 text-[10px] font-medium bg-emerald-50 text-emerald-700 rounded border border-emerald-200">
//                   <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
//                   Machine & Final Target Optional
//                 </span>
//               )}
//               <button onClick={() => { setStep(1); setPlanItems([]); setActiveTab('workOrders'); }}
//                 className="px-3 py-1.5 text-xs font-medium text-gray-600 bg-white hover:bg-gray-50 rounded-lg border border-gray-300 transition-colors flex items-center gap-1.5">
...
//             {/* Planned Tab: only for dry-only permission users */}
//             {showPlannedTab && (
//               <button onClick={() => setActiveTab('planned')}
//                 className={`flex-1 px-6 py-3 text-sm font-semibold border-b-2 transition-all flex items-center justify-center gap-2 ${activeTab === 'planned' ? 'border-indigo-500 text-indigo-700 bg-indigo-50/30' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
//                 <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
...
//                     className="w-full border border-gray-200 rounded-lg pl-9 pr-9 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-gray-50 disabled:cursor-not-allowed placeholder:text-gray-400" />
//                   {searchLoading && (
//                     <svg className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin h-4 w-4 text-indigo-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
//                       <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
//                       <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
//                     </svg>
//                   )}
...
//                         <tr>
//                           <th className="px-3 py-2.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider w-8"></th>
//                           <th className="px-2 py-2.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider w-16">WO No</th>
//                           <th className="px-3 py-2.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider w-28">Style / Color</th>
//                           <th className="px-3 py-2.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider w-28">Buyer / Marks</th>
//                           <th className="px-3 py-2.5 text-center text-[10px] font-bold text-gray-500 uppercase tracking-wider w-16">Ord Qty</th>
//                           <th className="px-3 py-2.5 text-center text-[10px] font-bold text-gray-500 uppercase tracking-wider w-16">Balance</th>
//                           {/* ★ Show "Machines (Optional)" header when user has Both Perms and is in Dry Stage */}
//                           <th className="px-3 py-2.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider w-48">
//                             {isMachineOptional ? (
//                               <span className="flex items-center gap-1">
//                                 Machines
//                                 <span className="text-[9px] font-medium text-emerald-600 normal-case tracking-normal">(optional)</span>
//                               </span>
//                             ) : 'Machines'}
//                           </th>
//                           {isWashStage && (
...
//                             </>
//                           )}
//                           {/* ★ For Dry-only users, they input Final Target here instead of it being hidden/auto-calculated */}
//                           {(!isWashStage && showPlannedTab) && (
//                             <th className="px-3 py-2.5 text-center text-[10px] font-bold text-gray-500 uppercase tracking-wider w-20">Final Tgt</th>
//                           )}
//                           {isWashStage && (
...
//                                   className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer" />
//                               </td>
//                               <td className="px-3 py-2"><span className="font-bold text-indigo-600 text-xs">{order.workOrderNo}</span></td>
//                               <td className="px-3 py-2">
//                                 <div className="text-xs text-gray-900 font-medium truncate max-w-[100px]" title={order.styleName}>{order.styleName}</div>
//                                 <div className="text-[10px] text-gray-500 mt-0.5 truncate max-w-[100px]" title={order.color}>{order.color}</div>
//                               </td>
//                               <td className="px-3 py-2">
//                                 <div className="text-xs text-gray-700 truncate max-w-[80px]" title={order.buyer}>{order.buyer}</div>
//                                 <div className="text-[10px] text-gray-400 mt-0.5 truncate max-w-[80px]" title={order.marks}>{order.marks || '-'}</div>
//                               </td>
//                               <td className="px-3 py-2 text-center text-xs text-gray-700">{order.orderQuantity?.toLocaleString()}</td>
//                               <td className="px-3 py-2 text-center">
//                                 <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-bold ${(order.washBalance ?? 0) > 0 ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'}`}>
...
//                                   }`}>
//                                   <span className="truncate">
//                                     {added && item?.selectedMachines?.length > 0 ? item.selectedMachines.map(m => m.machineCode).join(', ') : (isMachineOptional ? 'Optional...' : 'Select...')}
//                                   </span>
//                                   <svg className="w-3 h-3 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
//                                 </button>
//                                 {added && item?.selectedMachines?.length > 0 && (
...
//                               <td className="px-3 py-1.5">
//                                 {isWashStage ? (
//                                   <div className={`w-full border rounded px-2 py-1 text-xs text-center font-bold min-h-[26px] flex items-center justify-center ${item?.baseTargetQty > 0 ? 'border-indigo-200 text-indigo-700 bg-indigo-50' : 'border-gray-200 text-gray-400 bg-gray-50'}`} title={!item?.baseTargetQty && added ? 'Select machines to auto-calculate' : ''}>
//                                     {item?.baseTargetQty > 0 ? item.baseTargetQty : (added ? '0' : '-')}
//                                   </div>
//                                 ) : showPlannedTab ? (
//                                   // Dry-only: Locked, shows value passed down from wash plan
//                                   <div className="w-full border border-gray-200 rounded px-2 py-1 text-xs text-center font-bold text-blue-700 bg-blue-50 cursor-not-allowed min-h-[26px] flex items-center justify-center">
//                                     {item?.baseTargetQty || 0}
//                                   </div>
//                                 ) : (
//                                   // Both Access + Dry Stage: User manually inputs base target
//                                   <input type="number" min="0" value={item?.baseTargetQty ?? ''}
//                                     onChange={(e) => { ensureInPlan(order); handleBaseTargetChange(order.workOrderNo, e.target.value); }}
//                                     onFocus={() => ensureInPlan(order)} placeholder="0"
//                                     className="w-full border border-gray-200 rounded px-2 py-1 text-xs text-center font-semibold focus:outline-none focus:ring-1 focus:ring-indigo-500 hover:border-gray-300 transition-colors" />
//                                 )}
...
//                               {/* Allow selecting machines directly in the planned tab */}
//                               <td className="px-3 py-1.5">
//                                 <button type="button" ref={el => { dropdownBtnRefs.current[wp.workOrderNo] = el; }}
//                                   onClick={(e) => { addFromPlanned(wp); toggleDropdown(wp.workOrderNo, e); }}
//                                   className={`w-full border rounded px-2 py-1 text-left text-[11px] font-medium transition-colors flex items-center justify-between gap-1 ${
//                                     alreadyAdded && item?.machineIds?.length > 0 ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-500 hover:border-indigo-300'
//                                   }`}>
//                                   <span className="truncate">
//                                     {alreadyAdded && item?.selectedMachines?.length > 0 ? item.selectedMachines.map(m => m.machineCode).join(', ') : 'Select...'}
//                                   </span>
//                                   <svg className="w-3 h-3 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
//                                 </button>
//                                 {alreadyAdded && item?.selectedMachines?.length > 0 && (
//                                   <div className="flex flex-wrap gap-0.5 mt-1">
//                                     {item.selectedMachines.map(m => (
//                                       <span key={m.id} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-indigo-100 text-indigo-700 rounded text-[9px] font-semibold">
//                                         {m.machineCode}
//                                         <button type="button" onClick={() => removeMachineTag(wp.workOrderNo, m.id)} className="ml-0.5 text-indigo-400 hover:text-indigo-600">
//                                           <svg className="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
//                                         </button>
//                                       </span>
//                                     ))}
...
//                               {/* Allow inputting the Final Target directly in the planned tab */}
//                               <td className="px-3 py-1.5">
//                                 <input type="number" min="0" value={item?.finalTargetQty ?? ''}
//                                   onChange={(e) => { addFromPlanned(wp); handleFinalTargetChange(wp.workOrderNo, e.target.value); }}
//                                   onFocus={() => addFromPlanned(wp)} placeholder="0"
...
//                       <tbody className="divide-y divide-gray-100">
//                         {planItems.map(item => {
//                           // Validation inside map
//                           const incomplete = isMachineOptional 
//                             ? (!item.baseTargetQty || item.baseTargetQty <= 0) 
//                             : showPlannedTab 
//                               ? (!item.baseTargetQty || item.baseTargetQty <= 0 || !item.finalTargetQty || item.finalTargetQty <= 0 || item.machineIds.length === 0)
//                               : ((!item.machineIds || item.machineIds.length === 0) || !item.baseTargetQty || item.baseTargetQty <= 0);
...
//                                 <span className={`text-xs font-bold ${(item.washBalance ?? 0) > 0 ? 'text-orange-600' : 'text-green-600'}`}>{item.washBalance?.toLocaleString()}</span>
//                               </td>
//                               <td className="px-3 py-2 text-center text-xs font-semibold">{item.baseTargetQty?.toLocaleString() || <span className="text-red-500">-</span>}</td>
//                               {isWashStage && (
//                                 <>
//                                   <td className="px-3 py-2 text-center text-xs text-gray-600">{item.percentage}%</td>
//                                   <td className="px-3 py-2 text-center text-xs font-bold text-amber-700">{item.adjustedTargetQty?.toLocaleString() || '0'}</td>
...
//                               {/* Final Target display in Selected Tab */}
//                               {((isWashStage) || (!isWashStage && showPlannedTab)) && (
//                                 <td className="px-3 py-2 text-center text-xs font-bold text-green-700">{item.finalTargetQty?.toLocaleString() || <span className="text-red-500">-</span>}</td>
//                               )}
...
//                                   ))}</div>
//                                 ) : (
//                                   <span className={`text-[10px] font-semibold ${isMachineOptional ? 'text-gray-400' : 'text-red-500'}`}>
//                                     {isMachineOptional ? '—' : 'No machine'}
//                                   </span>
//                                 )}
//                               </td>
//                               <td className="px-3 py-2 text-center">
...
//                       <p className="text-xs font-semibold text-amber-800">
//                         {isMachineOptional
//                           ? 'Incomplete items — ensure Base Target has a value for all items'
//                           : showPlannedTab
//                             ? 'Incomplete items — ensure Final Target and Machines are selected for all items'
//                             : 'Incomplete items — go back to Work Orders tab to fill missing fields'}
//                       </p>
//                     </div>
//                   )}
...
//       {/* ── Fixed Position Machine Dropdown ── */}
//       {openDropdownFor && machines.length > 0 && (
//         <div id="machine-dropdown" className="fixed z-[9999] w-72 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden" style={{ top: dropdownPos.top, left: dropdownPos.left }}>
//           <div className="p-2 bg-gray-50 border-b border-gray-200">
//             <input type="text" placeholder="Filter machines..."
//               value={machineSearchTerms[openDropdownFor] || ''}
